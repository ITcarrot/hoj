...
void parse_args(int argc, char **argv) {
	...
	if (run_program_config.type == "python2.7") {
		...
		string pre[3] = {"/usr/bin/python3.6", "-I", "-B"};
...
RunResult trace_children() {
	...
	while (true) {
		...
		if (usertim > run_program_config.time_limit * 1000) {
			stop_all();
			return RunResult(RS_TLE, usertim, usermem);
		}
		if (usermem > run_program_config.memory_limit * 1024) {
			stop_all();
			return RunResult(RS_MLE, usertim, usermem);
		}
		...
		if (WIFEXITED(stat)) {
			...
			if (rp_children[0].mode == -1) {
				stop_all();
				return RunResult(RS_JGF, usertim, usermem, WEXITSTATUS(stat));
			} else {
		...
		if (WIFSIGNALED(stat)) {
			...
			if (pid == rp_children[0].pid) {
				...
				case SIGXCPU: // nearly impossible
					stop_all();
					return RunResult(RS_TLE, -1, usermem);
				case SIGXFSZ:
					stop_all();
					return RunResult(RS_OLE, usertim, usermem);
				default:
					stop_all();
					return RunResult(RS_RE, usertim, usermem, WTERMSIG(stat));
				}
			} else {
				rp_children_del(pid);
				continue;
				...
				if ((p == 0 && sig == SIGTRAP) || (p != 0 && sig == SIGSTOP)) {
					if (p == 0) {
						...
						if (ptrace(PTRACE_SETOPTIONS, pid, NULL, ptrace_opt) == -1) {
							stop_all();
							return RunResult(RS_JGF, usertim, usermem);
						}
					}
					sig = 0;
				}
				...
				if (rp_children[p].mode == 0) {
					if (run_program_config.safe_mode) {
						if (!check_safe_syscall(pid, run_program_config.need_show_trace_details)) {
							stop_all();
							return RunResult(RS_DGS, usertim, usermem);
						}
					}
