...
const RunLimit RL_JUDGER_DEFAULT = RunLimit(700, 2048, 512);
...
const RunLimit RL_CUSTOM_TEST = RunLimit(20, 2048, 64);
...
int max_memory = 0;
int tot_score  = 0;
ostringstream details_out,short_details_out;
...
void add_report_info(const PointInfo &info) {
	short_details_out << ',' << info.num << ':' << htmlspecialchars(info.info);
}
void add_point_info(const PointInfo &info, const bool isAC = false) {
	if (info.num >= 0 && isAC) {
		if(info.ust >= 0) {
			tot_time += info.ust;
		}
...
void report_judge_status(const char *status) {
	...
	if (flock(fileno(f), LOCK_EX) != -1) {
		if (ftruncate(fileno(f), 0) != -1) {
			fprintf(f, "%s%s\n", status, short_details_out.str().c_str());
			fflush(f);
		}
...
RunCheckerResult run_checker(
	...
	res.info = file_preview(result_path + "/checker_error.txt",500);
	return res;
}
...
RunCompilerResult run_compiler(const char *path, ...) {
	...
	if (!res.succeeded) {
		if (ret.type == RS_AC) {
			res.info = file_preview(result_path + "/compiler_result.txt", 10000);
...
RunCompilerResult compile_c(const string &name, const string &path = work_path) {
	string optimized = conf_str(string(name) + "_optimized", "-O0");
	string cstandard = conf_str(string(name) + "_cstandard", "-std=c11");
	return run_compiler(path.c_str(), "/usr/bin/gcc", "-o", name.c_str(), "-x", "c", (name + ".code").c_str(),
		"-lm", "-DONLINE_JUDGE", "-Wall", "-Wno-unused-result", optimized.c_str(), cstandard.c_str(), NULL);
}
RunCompilerResult compile_pas(const string &name, const string &path = work_path) {
	string optimized = conf_str(string(name) + "_optimized", "-O-");
	return run_compiler(path.c_str(), "/usr/bin/fpc", (name + ".code").c_str(), optimized.c_str(), NULL);
}
RunCompilerResult compile_cpp(const string &name, const string &path = work_path) {
	string optimized = conf_str(string(name) + "_optimized", "-O0");
	string cstandard = conf_str(string(name) + "_cstandard", "-std=c++03");
	return run_compiler(path.c_str(), "/usr/bin/g++", "-o", name.c_str(), "-x", "c++", (name + ".code").c_str(),
		"-lm", "-DONLINE_JUDGE",  "-Wall" , "-Wno-unused-result", optimized.c_str(), cstandard.c_str(), NULL);
}
...
RunCompilerResult compile_python3(const string &name, const string &path = work_path) {
	return run_compiler(path.c_str(),
			"/usr/bin/python3.6", "-I", "-B", "-O", "-c", ("import py_compile\nimport sys\ntry:\n    py_compile.compile('" + name + ".code'" + ", '" + name + "', doraise=True)\n    sys.exit(0)\nexcept Exception as e:\n    print(e)\n    sys.exit(1)").c_str(), NULL);
}

RunCompilerResult compile(const char *name)  {
	string lang = conf_str(string(name) + "_language");

	if ((lang == "C++" || lang == "C") && has_illegal_keywords_in_file(work_path + "/" + name + ".code"))
	{
		RunCompilerResult res;
		res.type = RS_DGS;
		res.ust = -1;
		res.usm = -1;
		res.succeeded = false;
		res.info = "Illegal Keywords Found";
		return res;
	}
...
RunCompilerResult compile_c_with_implementer(const string &name, const string &path = work_path) {
	string optimized = conf_str(string(name) + "_optimized", "-O0");
	string cstandard = conf_str(string(name) + "_cstandard", "-std=c11");
	return run_compiler(path.c_str(), "/usr/bin/gcc", "-o", name.c_str(), "implementer.c", "-x", "c", (name + ".code").c_str(),
		"-lm", "-DONLINE_JUDGE",  "-Wall" , "-Wno-unused-result", optimized.c_str(), cstandard.c_str(), NULL);
}
RunCompilerResult compile_pas_with_implementer(const string &name, const string &path = work_path) {
	...
	string optimized = conf_str(string(name) + "_optimized", "-O-");
	return run_compiler(path.c_str(), "/usr/bin/fpc", "implementer.pas", ("-o" + name).c_str(), optimized.c_str(), NULL);
}
RunCompilerResult compile_cpp_with_implementer(const string &name, const string &path = work_path) {
	string optimized = conf_str(string(name) + "_optimized", "-O0");
	string cstandard = conf_str(string(name) + "_cstandard", "-std=c++03");
	return run_compiler(path.c_str(), "/usr/bin/g++", "-o", name.c_str(), "implementer.cpp", "-x", "c++", (name + ".code").c_str(),
		"-lm", "-DONLINE_JUDGE",  "-Wall" , "-Wno-unused-result", optimized.c_str(), cstandard.c_str(), NULL);
}
...
PointInfo test_point(const string &name, const int &num, TestPointConfig tpc = TestPointConfig()) {
	...
	if (!tpc.submit_answer) {
		pro_ret = run_submission_program(
				...
				name);
		if(pro_ret.ust>conf_int("time_limit_ms", num, conf_int("time_limit", num, RL_DEFAULT.time)*1000)){
			pro_ret.type=RS_TLE;
		}
		if (conf_has("token")) {
		...
		if (pro_ret.type != RS_AC) {
			return PointInfo(num, 0, pro_ret.ust, pro_ret.usm,
					info_str(pro_ret.type),
					file_preview(tpc.input_file_name), file_preview(tpc.output_file_name),
					(pro_ret.exit_code > 0 ? ("Error with return value " + vtos(pro_ret.exit_code)) : ""));
		}
	} else {
		pro_ret.type = RS_AC;
		pro_ret.ust = -1;
...
CustomTestInfo ordinary_custom_test(const string &name) {
	...
	return CustomTestInfo(pro_ret.ust, pro_ret.usm, 
			(pro_ret.type == RS_AC ? "Success" : info_str(pro_ret.type)), 
			(pro_ret.exit_code > 0 ? ("Error with return value " + vtos(pro_ret.exit_code)) : ""), 
			file_preview(output_file_name, 30000));
}
