...
class HTML {
	...
	public static function pruifier() {
		...
		$def->addAttribute('span', 'data-email', 'CDATA');
		$def->addAttribute('span', 'data-super', 'Enum#0,1');
		
		$def->addElement('video',  'Inline', 'Inline', 'Common');
		$def->addAttribute('video', 'autoplay', 'Bool#autoplay');
		$def->addAttribute('video', 'controls', 'Bool#controls');
		$def->addAttribute('video', 'loop', 'Bool#loop');
		$def->addAttribute('video', 'preload', 'Bool#preload');
		$def->addAttribute('video', 'poster', 'URI');
		$def->addAttribute('video', 'src', 'URI');
		
		$def->addElement('object',  'Block', 'Flow', 'Common');
		$def->addAttribute('object', 'classid', 'CDATA');
		$def->addAttribute('object', 'codebase', 'URI');
		$def->addElement('param',  'Block', 'Empty', 'Common');
		$def->addAttribute('param', 'name', 'CDATA');
		$def->addAttribute('param', 'value', 'CDATA');
		$def->addElement('embed',  'Block', 'Inline', 'Common');
		$def->addAttribute('embed', 'src', 'URI');
		$def->addAttribute('embed', 'pluginspage', 'URI');
		$def->addAttribute('embed', 'quality', 'CDATA');
		$def->addAttribute('embed', 'type', 'CDATA');
		
		$def->addAttribute('section', 'data-background', 'CDATA');
		$def->addAttribute('section', 'data-background-repeat', 'Bool#repeat');
		$def->addAttribute('section', 'data-background-size', 'CDATA');
		$def->addAttribute('section', 'data-background-video', 'URI');
		$def->addAttribute('section', 'data-background-color', 'Color');
		$def->addAttribute('section', 'data-background-image', 'URI');
		$def->addAttribute('section', 'data-background-transition', 'CDATA');
		$def->addAttribute('section', 'data-transition', 'CDATA');
