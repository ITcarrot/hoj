<?php
	...
	if (!hasContestPermission($myUser, $contest)) {
		if ($contest['cur_progress'] == CONTEST_NOT_STARTED) {
			...
			if ($myUser && !hasRegistered($myUser, $contest)) {
			header("Location: /contest/{$contest['id']}/register");
			die();
			}
		}
	...
	if(!Auth::id()||(isContestUser(Auth::user())&&$contest['is_open']!=1)){
		$cur_tab='standings';
	}
	// problems: pos => id
	...
	function queryContestData() {
		...
		if ($contest['cur_progress'] < CONTEST_FINISHED) {
			$result = DB::query("select id, submit_time, submitter, problem_id, score, used_time from submissions where contest_id = {$contest['id']} and score is not null order by id");
		} else {
			$result = DB::query("select submission_id, date_add('{$contest['start_time_str']}', interval penalty second), submitter, problem_id, score, used_time from contests_submissions where contest_id = {$contest['id']}");
		}
		while ($row = DB::fetch($result, MYSQLI_NUM)) {
			$row[0] = (int)$row[0];
		...
		while ($row = DB::fetch($result, MYSQLI_NUM)) {
			$row[1] = (int)$row[1];
			$person = queryUser($row[0]);
			$row[2] = HTML::escape($person['email']);
			$row[3] = $person['usergroup'] == 'S' ? 1 : 0;
			$people[] = $row;
		}
...
/*			$penalty = (new DateTime($submission[1]))->getTimestamp() - $contest['start_time']->getTimestamp();
			...
			if($contest['extra_config']['standings_version'] >= 2){
				if($submission[4]==0){
					$score[$submission[2]][$submission[3]] = array($submission[4], 0, $submission[0]);
				}else{
					$score[$submission[2]][$submission[3]] = array($submission[4], $submission[5], $submission[0]);
				}
			}else{
				$penalty = (new DateTime($submission[1]))->getTimestamp() - $contest['start_time']->getTimestamp();
				if(!isset($score[$submission[2]][$submission[3]])){
					...
					$times=$score[$submission[2]][$submission[3]][3];
				}
				$penalty+=$times*1200;
				if ($submission[4] == 0) {
					$penalty = 0;
				}
				$score[$submission[2]][$submission[3]] = array($submission[4], $penalty, $submission[0],$times+1);
			}
		}
			...
			for ($i = 0; $i < $n_problems; $i++) {
				if (isset($score[$person[0]][$i])) {
					...
					if ($update_contests_submissions) {
						DB::insert("insert into contests_submissions (contest_id, submitter, problem_id, submission_id, score, penalty, used_time) values ({$contest['id']}, '{$person[0]}', {$contest_data['problems'][$i]}, {$cur_row[2]}, {$cur_row[0]}, {$cur_row[1]}, {$cur_row[1]})");
					}
				}
			}
			$standings[] = $cur;
	...
	if (hasContestPermission($myUser, $contest)) {
		if (CONTEST_PENDING_FINAL_TEST <= $contest['cur_progress']) {
			...
			$start_test_form->handle = function() {
				...
				DB::query("delete from contests_submissions where contest_id = {$contest['id']}");
				DB::query("update contests set status = 'testing' where id = {$contest['id']}");
				DB::manage_log('contest','rejudge contest '.$contest['id']);
			};
...
EOD;
				...
				DB::manage_log('contest','publish contest '.$contest['id'].' result');
			};
	...
	function echoDashboard() {
		...
		for ($i = 0; $i < count($contest_problems); $i++) {
			...
			echo chr(ord('A') + $i), '</td>';
			echo '<td>', getContestProblemLink($problem, $contest['id']);
			if(isset($contest['extra_config']["problem_{$problem['id']}"])){
				echo '&nbsp;&nbsp;<span class="badge" style="background:green">本题提交结果就是最终结果</span>';
			}
			echo '</td></tr>';
		}
		echo '</tbody>';
...
<div class="row">
	...
	<div class="col-sm-9">
	<?php endif ?>
		<?php if(Auth::id()&&!(isContestUser(Auth::user())&&$contest['is_open']!=1)):?>
		<?= HTML::tablist($tabs_info, $cur_tab) ?>
		<?php endif ?>
		<div class="top-buffer-md">
		<?php
			if ($cur_tab == 'dashboard' && $myUser!=NULL) {
				echoDashboard();
			} elseif ($cur_tab == 'submissions' && $myUser!=NULL) {
				echoMySubmissions();
			} elseif ($cur_tab == 'standings') {
				echoStandings();
			}else{
				echo '<h4>在查看本页面前请先登录</h4>';
			}
		?>
		</div>
	</div>
