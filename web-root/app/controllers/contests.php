...
EOD;
		...
		echo '<td>', '<a href="/contest/'.$contest['id'].'">'.$contest['start_time_str'].'</a>', '</td>';
...
<div class="row">
	<div class="col-sm-6"><h3><?= UOJLocale::get('contests::current or upcoming contests') ?></h3></div>
	<?php if(isSuperUser($myUser)):?>
	<div class="col-sm-6 text-right top-buffer-md"><a href="/contest/new" class="btn btn-primary"><?=UOJLocale::get('contests::add new contest')?></a></div>
	<?php endif ?>
</div>
<?php
	$table_header = '';
	$table_header .= '<tr>';
	$table_header .= '<th>比赛编号</th>';
...
<h3><?= UOJLocale::get('contests::ended contests') ?>
<a role="button" class="btn btn-info pull-right" href="/contest/search"><span class="glyphicon glyphicon-stats"></span> 统计</a>
</h3>
<?php
	echoLongTable(array('*'), 'contests', "status = 'finished'", 'order by id desc', $table_header,
		'echoContest',
