<?php
	...
	function customTestSubmissionJudged() {
		$submission = DB::selectFirst("select submitter, status, content, result from custom_test_submissions where id = {$_POST['id']}");
		if ($submission == null) {
			return;
		}
	...
	function hackJudged() {
		...
		if ($ok) {
			...
			if ($result['score']) {
				$owner = DB::selectFirst("select submission_id, owner from hacks where id = {$_POST['id']}");
				sendSystemMsg($owner['owner'], "你被HACK了", "你被HACK了，提交记录<a href=\"/submission/{$owner['submission_id']}\">{$owner['submission_id']}</a>。");
	...
	if (isset($_POST['submit'])) {
		...
		$_POST['result'] = mb_convert_encoding($_POST['result'],'UTF-8');
		if(!json_decode($_POST['result'], true))
			$_POST['result']='{"status": "Judged", "time": 0, "error": "Result Error", "score": 0, "details": "<error>评测机返回的信息中包括了非法字符，请检查你是否输出了奇奇怪怪的符号（非英文字母），或是让编译器返回了奇奇怪怪的信息</error>\\n", "memory": 0}';
	...
	function queryCustomTestSubmissionToJudge() {
		global $submission;
		$submission = DB::selectFirst("select id, content from custom_test_submissions where judge_time is null order by id limit 1");
		if ($submission) {
			DB::update("update custom_test_submissions set judge_time = now(), status = 'Judging', judger='{$_POST['judger_name']}' where id = {$submission['id']} and judge_time is null");
			if (DB::affected_rows() != 1) {
				$submission = null;
			}
		}
	...
	function queryHackToJudge() {
		...
		if ($hack) {
			DB::update("update hacks set judge_time = now(), judger='{$_POST['judger_name']}' where id = {$hack['id']} and judge_time is null");
			if (DB::affected_rows() != 1) {
				$hack = null;
			}
		}
	...
	function findSubmissionToJudge() {
		...
		querySubmissionToJudge('Waiting', "judge_time = now(), status = 'Judging',judger='$judger'");
		if ($submission) {
			return true;
		}
		...
		querySubmissionToJudge('Judged, Waiting', "status = 'Judged, Judging',judger='$judger'");
		if ($submission) {
			return true;
		}
		
		querySubmissionToJudge('Waiting Rejudge', "judge_time = now(), status = 'Judging',judger='$judger'");
		if ($submission) {
			return true;
		}
		
		querySubmissionToJudge('Waiting At Tail', "judge_time = now(), status = 'Judging',judger='$judger'");
		if ($submission) {
			return true;
		}
