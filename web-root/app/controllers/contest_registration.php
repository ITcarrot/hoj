...
<h1>比赛规则</h1>
<ul><h4>
	<li>比赛报名后不算正式参赛，报名后进了比赛页面也不算参赛，看了题目也不算参赛，<font class="text-danger">提交代码后（包括CE的代码）才算正式参赛</font>。如果未正式参赛则不算rating。</li>
	<li>比赛中请使用标准输入输出</li>
	<li>每题得分以最后一次提交为准，Compile Error不计提交次数</li>
	<li>比赛排名按总分为第一关键字，本次比赛排名的第二关键字为<font class="text-danger"><?=$contest['extra_config']['standings_version'] >= 2 ? '程序总运行时间' : '罚时' ?></font></li>
	<li>如无特别说明，比赛期间提交结果<font class="text-danger">不是最终结果</font></li>
