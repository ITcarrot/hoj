<?php
	global $uojSupportedLanguages;
	$uojSupportedLanguages = array('C++', 'C', 'Python2.7', 'Python3', 'Pascal');
	...
	function rejudgeProblem($problem) {
		...
		DB::manage_log('problems','rejudge problem '.$problem['id']);
	}
	function rejudgeProblemAC($problem) {
		...
		DB::manage_log('problems','rejudge problemAC '.$problem['id']);
	}
	function rejudgeSubmission($submission) {
		...
		DB::manage_log('problems','rejudge submission '.$submission['id']);
	}
	...
	function updateBestSubmission($username,$problem_id)
	{
		$best=DB::selectFirst("select * from best_submissions where submitter='$username' and problem_id=$problem_id");
		$user=DB::selectFirst("select liveness,liveness_day from user_info where username='$username'");
		$score=DB::selectFirst("select score from submissions where submitter='$username' and problem_id=$problem_id order by score desc limit 1",MYSQLI_NUM)[0];
		if($score>$best['score']){
			...
			if($today-$user['liveness_day']>30)
				$user['liveness']=0;
			$user['liveness']=(int)($user['liveness']+$score-$best['score']);
			DB::query("update user_info set liveness={$user['liveness']},liveness_day=$today where username='$username'");
		}
		DB::query("delete from best_submissions where submitter='$username' and problem_id=$problem_id");
		DB::query("insert into best_submissions (submitter,problem_id,score) value('$username',$problem_id,$score)");
	}
