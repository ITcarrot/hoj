...
function isSuperUser($user) {
	return $user['username']=='std' || ($user != null && $user['usergroup'] == 'S');
}
function isContestUser($user) {
	return $user != null && $user['usergroup'] == 'C';
}
function getProblemExtraConfig($problem) {
	$extra_config = json_decode($problem['extra_config'], true);
	
	$default_extra_config = array(
		'view_content_type' => 'ALL_AFTER_AC',
		'view_all_details_type' => 'ALL',
		'view_details_type' => 'ALL_AFTER_AC'
	);
