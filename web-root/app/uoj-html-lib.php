	...
	$res = preg_replace_callback('/@(@|[a-zA-Z0-9_]{1,20})/', function($matches) use(&$referrers) {
		if ($matches[1] === '@') {
			...
			if ($user == null) {
				...
				return '<span class="uoj-username" data-rating="'.$user['rating'].'" data-email="@'.$esc_email.'" data-super="'.($user['usergroup']=='S' ? '1' : '0').'">@'.$user['username'].'</span>';
			}
		}
	}, $str);
...
function getUserLink($username, $rating = null) {
	if (validateUsername($username) && ($user = queryUser($username))) {
		...
		$esc_email = HTML::escape($user['email']);
		return '<span class="uoj-username" data-rating="'.$rating.'" data-email="'.$esc_email.'" data-super="'.($user['usergroup']=='S' ? '1' : '0').'">'.$username.'</span>';
	} else {
...
function getSubmissionStatusDetails($submission) {
	...
	if ($out_status == 'Judged') {
		...
		if ($submission['status_details'] != '') {
			$status_img = $fly;
			$status_text = HTML::escape(explode(',',$submission['status_details'])[0]).' by '.$submission['judger'];
		} else  {
			$status_img = $think;
		...
		if($submission['submitter']===Auth::id()){
			...
			$html .= '<div class="text-center">' . $status_text . '</div>';	
		}
...
function echoSubmission($submission, $config, $user) {
	...
	$show_status_details = $status !== 'Judged';
	...
	if (!isset($config['judge_time_hidden']))
		echo '<td class="hidden-xs"><small>', $submission['judge_time'], '</small></td>';
	echo '</tr>';
	if ($show_status_details) {
...
function echoSubmissionsList($cond, $tail, $config, $user) {
	...
	if (!isset($config['judge_time_hidden'])) {
		$header_row .= '<th class="hidden-xs">'.UOJLocale::get('problems::judge time').'</th>';
...
function echoSubmissionContent($submission, $requirement) {
	...
	foreach ($requirement as $req) {
		if ($req['type'] == "source code") {
			...
			if($file_language == 'C++' || $file_language == 'C'){
				if($file_language == 'C')
					$lang_suf = '.c';
				else
					$lang_suf = '.cpp';
				do{
					$filename = UOJContext::storagePath().'/tmp/'.mt_rand().$lang_suf;
				}while(file_exists($filename));
				file_put_contents($filename,$file_content);
				exec("clang-format -style=file $filename >$filename.formatted");
				$formatted_content = file_get_contents($filename.'.formatted');
				unlink($filename);
				unlink($filename.'.formatted');
				$formatted_content = uojTextEncode($formatted_content, array('html_escape' => true , 'allow_CR' => true));
			}
			$file_content = uojTextEncode($file_content, array('html_escape' => true , 'allow_CR' => true));
			$footer_text = UOJLocale::get('problems::source code').', '.UOJLocale::get('problems::language').': '.$file_language;
			if($config["{$req['name']}_optimized"] || $config["{$req['name']}_cstandard"])
				$footer_text .= ', 编译选项：'. HTML::escape($config["{$req['name']}_optimized"]) .' '. HTML::escape($config["{$req['name']}_cstandard"]);
			...
			$id = 'src_code'.$submission['id'];
			if($formatted_content){
				echo '<button class="btn btn-xs btn-primary bot-buffer-sm for'.$id.'" data-clipboard-action="copy" data-clipboard-target="#'.$id.'" id="copy_btn">复制</button>';
				echo '<div style="float:right">';
				echo HTML::checkbox($id.'format',$submission['submitter']!=Auth::id());
				echo '</div>';
				echo '<pre id="'.$id.'"></pre>';
				echo '<script>';
				echo $id,'=',json_encode(array('<code class="'.$sh_class.'">'.$file_content."\n</code>", '<code class="'.$sh_class.'">'.$formatted_content."\n</code>")),';';
				if($submission['submitter']==Auth::id())
					echo "$('#$id').html({$id}[0]);";
				else
					echo "$('#$id').html({$id}[1]);";
				echo <<<EOD
$('#input-{$id}format').bootstrapSwitch({
	onText: "启用",
	onColor: 'success',
	offText: "禁用",
	offColor: 'danger',
...
$('#input-{$id}format').on('switchChange.bootstrapSwitch', function(e, state) {
	if(state){
		$('#{$id}').html({$id}[1]);
		sh_highlightDocument($('#{$id}')[0]);
	}else{
		$('#{$id}').html({$id}[0]);
		sh_highlightDocument($('#{$id}')[0]);
	}
...
EOD;
				...
				echo '<button class="btn btn-xs btn-primary bot-buffer-sm for'.$id.'" data-clipboard-action="copy" data-clipboard-target="#'.$id.'" id="copy_btn">复制</button><pre id="'.$id.'"><code class="'.$sh_class.'">'.$file_content."\n".'</code></pre>';
			}
			echo '</div>';
...
function echoHack($hack, $config, $user) {
	...
	if (!isset($config['judge_time_hidden']))
		echo '<td class="hidden-xs">', $hack['judge_time'], '</td>';
	echo '</tr>';
}
...
function echoRanklist($config = array()) {
	...
	$header_row .= '<th style="width: 17em;">'.UOJLocale::get('username').'</th>';
	$header_row .= '<th style="width: 48em;min-width:200px;">'.UOJLocale::get('motto').'</th>';
	$header_row .= '<th style="width: 5em;">'.UOJLocale::get('rating').'</th>';
	$header_row .= '<th style="width: 5em;">AC数</th>';
	$header_row .= '</tr>';

	$users = array();
	$print_row = function($user, $now_cnt) use(&$users) {
		if (!$users) {
			$rank = DB::selectCount("select count(*) from user_info where rating > {$user['rating']} and (rating!=1500 or username in (select username from contests_registrants where user_rating!=1500)) ") + 1;
		}
       ...
       $col_names = array('username', 'rating', 'motto', 'ac_num');
	$cond=" rating!=1500 or username in (select username from contests_registrants where user_rating!=1500) ";
	$tail = 'order by rating desc, ac_num desc, username asc';

	if (isset($config['top12'])) {
		$tail .= ' limit 16';
	}

	$config['get_row_index'] = '';
	echoLongTable($col_names, 'user_info', $cond, $tail, $header_row, $print_row, $config);
}
function getLivenessList()
{
	$today=(int)(time()/60/60/24);
	$before=$today-30;
	$re=DB::query("select username,liveness,liveness_day,motto from user_info where liveness_day>$before and liveness>0");
	$list=array();
	while($user=DB::fetch($re)){
		while($user['liveness_day']<$today&&$today-$user['liveness_day']<=30){
			$user['liveness']=(double)$user['liveness']*0.7;
			$user['liveness_day']++;
		}
		$list[]=array($user['username'],(int)($user['liveness']),$user['motto']);
	}
	usort($list,
		function($a,$b){
			if($b[1]==$a[1])
...
function getEncodeJS($id,$jump=0)
{
	$charset='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
	$_SESSION['spider']=uojRandString(mt_rand(20,30),$charset);
	$arr=array('#','_');
	for($i=0;$i<52;$i++){
		$arr[]=$charset[$i];
	}
	shuffle($arr);
	$str='<script>';
	$str.='var '.uojRandString(mt_rand(20,30),$charset).'="\\x77"';
	$map=array();
	foreach($arr as $val){
		$map[$val]=uojRandString(mt_rand(20,30),$charset);
		$str.=','.$map[$val].'="\\x'.dechex(ord($val)).'"';
	}
	$str.=';';
	$str_code=function($str)use($map){
		$len=strlen($str);
		$ret=$map[$str[0]];
		for($i=1;$i<$len;$i++){
			$ret.='+'.$map[$str[$i]];
	...
	if($jump==1){
		$form_str=uojRandString(mt_rand(20,30),$charset);
		$inp_str=uojRandString(mt_rand(20,30),$charset);
		$str.='var '.$form_str.'=window['.$str_code('document').']['.$str_code('createElement').']('.$str_code('form').');';
		$str.=$form_str.'['.$str_code('action').']=window['.$str_code('location')."][".$str_code('href')."];";
		$str.=$form_str.'['.$str_code('method').']='.$str_code('post').';';
		$str.=$form_str.'['.$str_code('style').']['.$str_code('display').']='.$str_code('none').';';
		$str.='var '.$inp_str.'=window['.$str_code('document').']['.$str_code('createElement').']('.$str_code('input').');';
		$str.=$inp_str.'['.$str_code('name').']='.$str_code('code').';';
		$str.=$inp_str.'['.$str_code('value').']='.$str_code($_SESSION['spider']).';';
		$str.=$form_str.'['.$str_code('appendChild').']('.$inp_str.');';
		$str.='window['.$str_code('document').']['.$str_code('head').']['.$str_code('appendChild').']('.$form_str.');';
		$str.=$form_str.'['.$str_code('submit').']();';
	}else{
		$str.='$['.$str_code('ajaxSettings').']['.$str_code('async').']=false;';
		$str.="$[".$str_code('post')."](window[".$str_code('location')."][".$str_code('href')."]";
		$str.=",{code:".$str_code($_SESSION['spider'])."}";
		$data_str=uojRandString(mt_rand(20,30),$charset);
		$str.=",function(".$data_str."){";
		$str.="$(".$str_code('#'.$id).")[".$str_code('html')."](".$data_str.");";
		$str.="$(".$str_code('#'.$id).")[".$str_code('uoj_highlight')."]();";
		$str.="window[".$str_code('sh_highlightDocument')."]();";
		$str.='window['.$str_code('MathJax').']['.$str_code('Hub').']['.$str_code('Queue').'](['.$str_code("Typeset").', window['.$str_code('MathJax').']['.$str_code('Hub').']]);';
		$str.="});";
		$str.='$['.$str_code('ajaxSettings').']['.$str_code('async').']=true;';
	}
	$str.='</script>';
	return $str;
}