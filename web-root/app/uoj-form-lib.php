		...
		$form->handle = function(&$vdata) use($form_name, $requirement, $zip_file_name_gen, $handle) {
			...
			foreach ($requirement as $req) {
				if ($req['type'] == "source code") {
					...
					if ($language == 'Pascal' && in_array($optimized, array('-O1','-O2','-O3','-O4')))
						$content['config'][] = array("{$req['name']}_optimized", $optimized);
					if ($language == 'C++' && in_array($cstandard, array('-std=c++17','-std=c++14','-std=c++11','-std=c++03','-std=c++98')))
						$content['config'][] = array("{$req['name']}_cstandard", $cstandard);
					if ($language == 'C' && in_array($cstandard, array('-std=c90','-std=c99','-std=c11')))
						$content['config'][] = array("{$req['name']}_cstandard", $cstandard);
				}
			}
