...
function showMessage(text, timeout, flag){
    if(flag || sessionStorage.getItem('waifu-text') === '' || sessionStorage.getItem('waifu-text') === null){
        ...
        $('.waifu-tips').stop();
        $('.waifu-tips').html(text).fadeIn(200);
...
function hideMessage(timeout){
    $('.waifu-tips').stop().css('opacity',1).css('display','block');
    if (timeout === undefined) timeout = 5000;
    window.setTimeout(function() {sessionStorage.removeItem('waifu-text')}, timeout);
    $('.waifu-tips').delay(timeout).fadeOut(200);
}
...
//draging
var div_bottom,div_left,mouse_x,mouse_y;
$('.waifu').mousedown(function(e){
	div_bottom=Number($(this).css('bottom').slice(0,-2));
	div_left=Number($(this).css('left').slice(0,-2));
	mouse_x=e.pageX;
	mouse_y=e.pageY;
	$(this).bind('mousemove',function(e){
		var bot = div_bottom - (e.pageY - mouse_y);
		var lft = div_left + (e.pageX - mouse_x);
		$(this).css({
			'bottom' : bot + 'px',
			'left' : lft + 'px'
		});
