function blog_editor_init(name, editor_config) {
	...
	if (editor_config.type == 'blog') {
		input_content_md.wrap('<div class="col-xs-6" style="padding:0;height:100%"></div>');
		input_content_md.parent().after('<div class="col-xs-6" id="blog-content-preview-sm" style="height:100%;overflow:auto"></div>');
		$('#blog-content-preview-sm').html(marked(input_content_md.val()));
	}
	...
	function send_content(post_data, config) {
		$.ajax({
			type : 'POST',
			data : post_data,
	...
	function save(config) {
		...
		if (editor_config.type == 'blog') {
			post_data[name+"_marked_content"] = marked(post_data[name+'_content_md']);
			send_content(post_data,config);
		} else if (editor_config.type == 'slide') {
			var pre_post_data ={};
			pre_post_data["pre-"+name] = '';
			pre_post_data[name+"_content_md"]=post_data[name+"_content_md"];
			pre_post_data['_token']=post_data['_token'];
			$.post(window.location.href,pre_post_data,function(data){
				try {
					data = JSON.parse(data)
				} catch (e) {
					showErrorHelp(name+'_content_md', data);
					if (config.need_preview) {
						set_preview_status(0);
					}
					config.fail();
				...
				for(i=0;i<data.length;i++)
					data[i]=marked(data[i]);
				post_data[name+"_marked_content"] = JSON.stringify(data);
				send_content(post_data,config);
			}).fail(function() {
				if (config.need_preview) {
					set_preview_status(0);
	...
	codeeditor.on('change', function() {
		...
		if (editor_config.type == 'blog') {
			...
			preview_timer=setTimeout(function(){
				$('#blog-content-preview-sm').html(marked(input_content_md.val()));
				$('#blog-content-preview-sm').uoj_highlight();
				sh_highlightDocument($('#blog-content-preview-sm')[0]);
	...
	if (editor_config.type == 'slide'){
		$("#input-" + name + "_theme").change(function() {
			set_saved(false);
		});
	}
